...
public class ClienteVH implements IViewHelper {
	...
	public EntidadeDominio getEntidade(HttpServletRequest request) {
		...
		if (operacao.equals("SALVAR")) {
			...
			cliente = criarCliente(request, Long.parseLong(request.getParameter("cliente")));
	...
	private Cliente criarCliente(HttpServletRequest request) {
		...
		cliente.setGenero(request.getParameter("genero").toString());
	...
	private Cliente criarCliente(HttpServletRequest request, long id) {
		Cliente cliente = new Cliente();
		cliente.setId(id);
		cliente.setNome(request.getParameter("nome"));
		cliente.setRg(request.getParameter("rg"));
	...
	public void setView(Resultado resultado, HttpServletRequest request, HttpServletResponse response)
		...
		if (resultado.getMsg().trim().equals("") && operacao.equals("ALTERAR")) {
//			resultado.setMsg("Alteração realizada com sucesso.");
		...
		if (!resultado.getMsg().trim().equals("")) {
			...
			if (operacao.equals("SALVAR")) {
				d = request.getRequestDispatcher("cadastro");
			}
